...
class Constrained_PPO(BaseController):
    ...
    def __init__(self,
                 ...
                 lamb_1=0,
                 lamb_2=0,
                 hard_constraint_scaler=0.6,
                 soft_constraint_scaler=0.05,
                 soft_constraint_allowance=3,
                 lambda_training_counter=1,
                 training=True,
        ...
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        # Set hard_constraint_scaler, lambda, learning rate for lambda's gradient descend, clipped parameter
        self.hard_constraint_scaler = hard_constraint_scaler
        self.soft_constraint_scaler = soft_constraint_scaler
        self.soft_constraint_allowance = soft_constraint_allowance
        self.lamb_1 = lamb_1
        self.lamb_2 = lamb_2
        self.soft_violations_counter = np.zeros(self.rollout_batch_size)
        self.policy_update_step = 0
        self.scale = scale
        # keep track of former action
        self.prev_act = np.zeros(self.rollout_batch_size)
        #self.lamb_lr = lamb_lr
        #self.clipped_para = clipped_para
        #self.lamb_gradient = 0
        self.lambda_training_counter = lambda_training_counter
        self.lambda_training=True
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        # Pre-/post-processing.
    ...
    def assign_lambda(self,lamb_1,lamb_2):
        self.lamb_1= lamb_1 
        self.lamb_2 =lamb_2
    def assign_steps(self,step):
        self.max_env_steps = step
    def reset(self):
        ...
        if self.training:
            ...
            #self.prev_act = [0] * self.rollout_batch_size
        else:
    ...
    def learn(self,
        ...
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        # reset lambda_gradient
        #self.lamb_gradient=0
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        while self.total_steps < self.max_env_steps:
            # Initialize a soft violations counter
            self.soft_violations_counter = np.zeros(self.rollout_batch_size)
            self.prev_act = np.zeros(self.rollout_batch_size)
            #self.env.TASK_INFO['trajectory_scale'] = np.random.uniform()
        ...
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        # Gradient descend on lambda.
        #self.lamb = torch.clamp(torch.tensor(self.lamb - self.lamb_lr*self.lamb_gradient), self.lamb*(1-self.clipped_para),self.lamb*(1+self.clipped_para)).item()
        #self.lamb -= self.lamb_gradient * self.lamb_lr 
        #print(f"current lambda")
        #print(self.lamb)
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
    ...
    def run(self,
        ...
        while len(ep_returns) < n_episodes:
            ...
            # record observation x on previous step
            pre_obs = obs[0]
            #----------------------------------------
    ...
    def train_step(self):
        ...
        for _ in range(self.rollout_steps):
            ...
            #---------------------------------------------------------------------------------------------
            #---------------------------------------------------------------------------------------------
            # Add hard constraint (gamma?)
            safety_indicator = np.fromiter((np.abs(next_obs[k, 0]) > self.hard_constraint_scaler for k in range(self.rollout_batch_size)), dtype=bool)
            soft_safety_indicator = np.fromiter((np.abs(act[k]-self.prev_act[k]) > self.soft_constraint_scaler for k in range(self.rollout_batch_size)), dtype=bool)
            # update the violation counter
            self.soft_violations_counter += soft_safety_indicator
            soft_penalty = np.fromiter((self.soft_violations_counter[k]==self.soft_constraint_allowance+1 for k in range(self.rollout_batch_size)), dtype=bool)
            rew = np.log(rew)
            # apply penalty for hard constraint and for soft constraint (only once)
            rew -= self.lamb_1 * safety_indicator + self.lamb_2 * soft_penalty 
            rew = np.exp(rew)
            # +1 to the counter that has already penalized the reward to make sure it happens only once
            self.soft_violations_counter += soft_penalty
            #rew = rew * safety_indicator + 1e-3
            #lamb_gradient -= np.mean(safety_indicator)
            #---------------------------------------------------------------------------------------------
            #---------------------------------------------------------------------------------------------
            #---------------------------------------------------------------------------------------------
        ...
        #---------------------------------------------------------------------------------------------
        #Accumulate gradient
        #self.lamb_gradient += lamb_gradient
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
    ...
    def log_step(self,
        ...
        self.logger.add_scalars(
            {
                'policy update step': self.policy_update_step,
                'lambda training step': self.lambda_training_counter,
                'hard_lambda': self.lamb_1,
                'soft_lambda' : self.lamb_2,
                'current env steps': self.max_env_steps,
                'trajectory scale' : self.scale,
                'current updating lambda': self.lambda_training
            },
            step,
            prefix='lambda'
        )
        #self.logger.add_scalars(
        #    {
        #        'total_energy': results['eval']['total_energy']
        #    },
        #   step,
        #    prefix='energy'
        #)
